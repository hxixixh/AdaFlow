...
class ControlEnv():
    def __init__(self,
        ...
        if cam_setting: 
            problem_name = self.problem_name + "_" + cam_setting
        else: 
            if random_cam: 
                problem_name = self.problem_name + "randcam"
            else: 
...
class SegmentationRenderEnv(OffScreenRenderEnv):
    ...
    def segmentation_to_rgb(self, seg_im, random_colors=False):
        ...
        if random_colors:
            ...
            inds = np.arange(256)
            rstate.shuffle(inds)
            seg_img = np.array(255.0 * cm.rainbow(inds[seg_im], 10)).astype(np.uint8)[..., :3].astype(np.uint8).squeeze(-2)
            print(seg_img.shape)
